...
Appium使用WebDriver的客户端-服务端协议(webdriver json wire protocol协议)，按照这种客户端-服务端架构,使用任意语言编写的客户端向服务端发送适当的HTTP请求即可驱动测试。
# 官方文档
...
1. Test Script 发送HTTP协议json格式信息到 Appium Server(4723端口)
2. Appium Server 发送Socket协议JSON格式 信息到 Bootstrap.jar(4724端口)
3. Bootstrap.jar 返回Socket协议信息JSON格式到 Appium Server
4. Appium Server 返回HTTP协议json格式信息到 Test Script
