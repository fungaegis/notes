...
WebDriver是完全W3C标准化的, 客户端与Remote Server(WebDriver)的通信使用了HTTP协议及The Webdriver wire protocol协议进行规范,将不关心客户端脚本使用的是什么语言编写,只需最终按照the webdriver wire protocol协议规范通过http进行交互即可
