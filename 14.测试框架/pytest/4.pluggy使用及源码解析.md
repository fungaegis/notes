...
class PluginManager(object):
    ...
    def register(self, plugin, name=None):
        ...
        self._name2plugin[plugin_name] = plugin  # 将插件类实例地址与插件类实例做映射
        self._plugin2hookcallers[plugin] = hookcallers = []  # 将插件类实例为key映射一个相同类下面的调用链
...
1. 
### 5.执行函数
执行函数实际上调用的是`callers.py的_multicall方法`,在添加规范函数/注册插件函数的时候实例化了`_HookCaller`类,`_HookCaller`类的实例属性`self.multicall = _multicall`
