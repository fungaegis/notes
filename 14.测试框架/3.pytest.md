...
- `pytest -m mark`: 通过标记(mark)运行
    ...
    - ex: `pytest -m "not smoke"` 运行带somke标记外的用例
    ```py
    @pytest.mark.smoke
    class TestDemo:
...
[也可以使用hook函数进行修改](#pytest_collection_modifyitems)
...
### 1.xfail_strict
[用法](#7.标记失败-xfail)
默认启用,标记为@pytest.mark.xfail但实际通过的测试用例被报告为失败
```ini
[pytest]
xfail_strict=true
```
...
def pytest_addoption(parser):
    parser.addoption("--ENV", action="store", default="test", help="please choose environment")
...
```
cmd: `pytest --ENV test` \ `pytest --ENV=test`
