...
一、Linux简介
	...
	I.安装
		...
		2.需要Linux的镜像文件
		3.在vm中安装Linux操作系统
		4.详细安装步骤参考Linux的安装指南

    II.远程连接Linux服务器
		1.Linux环境运行正常
		2.本地客户端安装有远程连接的工具如：shell、putty、scrt等
		3.设置Linux的环境，具体细节请参看视频
...
三、目录操作                Ctrl +shift +n 在Windows下新建文件夹
	1.新建目录=文件夹	 连级目录
...
四、文件的操作
	...
	6.查看文件的命令
		...
		head -10 install.log	#查看install.log文件的前十行内容，等同于head -n 10 install.log
		tail -10 install.log	#查看install.log文件的末尾十行内容，等同于tail -n 10 install.log 
...
五、压缩
	...
	压缩包：将目录和文件一起打包并压缩，压缩包源文件依旧存在，压缩包后缀有两种格式.tgz或者.tar.gz 
...
八、系统命令
	...
	2.重启命令
        ...
        修改root密码  输入 init 1 --输入passwd--输入密码--reboot
纯文本格式
