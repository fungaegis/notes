...
- `head -10 install.log`: 查看install.log文件的前十行内容(默认10行)，等同于`head -n 10 install.log`
...
- `service network-manager restart`: 重启所有的网络服务
...
- `chown root1:root2 Desktop`: 同时更改用户为root1，所属组为root2组
