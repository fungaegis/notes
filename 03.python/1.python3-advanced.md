...
- 字典的原理(hash map)：
    - 创建两个散列表，一个列表存key 一个存value，对字典的key进行hash,通过运算后得到的数值为该key和value的列表下标

- 集合的原理(hash set):
    - 创建一个散列表，对元素进行hash，通过运算后得到的数值为元素在列表的下标
...
d = decorator("manger")  # 先创建装饰器类实例


# 被装饰
class A:

    @d  # decorator("manger")(b)
    def b(self, num):
        print(num)
...
##### python3 GIL的工作原理
GIL的目的是让多个线程按照一定的顺序并发执行, 而不仅仅保证一个线程运行

GIL的本质是条件互斥锁, 使用条件变量与互斥所来保证

条件变量是利用线程间共享的全局变量来控制

流程:
- 线程1运行时, 其他线程挂起等待5毫秒(默认)
- 当线程1在时间内运行完毕
    1. 主动释放GIL
- 当线程1在时间内未运行完毕
    1. 其他线程在等待时间结束后, 将条件变量`gil_drop_request`改成1, 随后自己再次进入挂起等待状态. 
    2. 线程1发现条件变量为1会主动释放GIL
- 线程1释放完GIL后, 发送signal通知其他挂起的线程(由系统决定具体发送给谁), 线程1进入临时等待状态
- 被通知的线程, 获取GIL 进入运行状态, 并返回signal给线程1, 线程1进入挂起等待状态
### 7.3. 队列
...
| Queue.get(block=True，timeout=None) | 获取队列，block表示队列无任务时是否等待，timeout表示等待多少秒 |
| Queue.put(item，block=True，timeout=None) | 写入队列，block表示队列满任务时是否等待，timeout表示等待多少秒 |
...
## 9. 海象符
python3.8+才支持该特性, 该符号用于在赋值时可一起进行判断
```py
# 将a赋值为5并做判断
if (a := 5) < 10:
    print(a)

# 上面将a赋值为5, 当前将a不断-1, 每次开头运行
while (a := a-1) > 0:
    print(a)

# 在做判断时已进行赋值, 避免之前存在重复运行的情况
[n for x in range(10) if (n:=x) < 5]
```