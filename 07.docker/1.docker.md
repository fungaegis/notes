...
### 2.2 docker image常用命令

- 查看images下的帮助: `docker image --help`
- 从仓库中拉取镜像(默认拉取最新): `docker pull IMAGE_NAME:TAG`
- 删除本地镜像 镜像名:标签(可选): `docker rmi python:3-alpine`
...
- 列出容器
    ...
    - 查看全部容器: `docker ps -a`
- 启动容器
    - 构建指定镜像(不常用): `docker container run IMAGE_NAME`
    - (常用): `docker run -d IMAGE_NAME`
        - `-d`: 独立运行
...
- 操作容器
    ...
    - 停止并删除指定容器: `docker rm -f CONTAINER_ID`
    - 查看容器内部进程: `docker top CONTAINER_ID || CONTAINER_NAME`
        - 不推荐使用: `docker container top CONTAINER_ID or CONTAINER_NAME`
    - 查看容器资源使用情况: `docker stats CONTAINER_ID || CONTAINER_NAME`
    - 停止容器: `docker stop CONTAINER_ID || CONTAINER_NAME`
    - 重启容器: `docker restart CONTAINER_ID || CONTAINER_NAME`
    - 暂停容器运行: `docker pause CONTAINER_ID || CONTAINER_NAME`
    - 恢复暂停的容器运行: `docker unpause CONTAINER_ID || CONTAINER_NAME`
    - 容器详情: `docker inspect CONTAINER_ID || CONTAINER_NAME`
    - 容器日志: `docker logs CONTAINER_ID || CONTAINER_NAME`
...
### 3.1 常用操作
- 挂载:`docker run -v $PWD/html:/usr/share/nginx/html IMAGE_NAME`
    - -v: 宿主机目录:容器目录:模式(可选)
        - 模式:(默认读写)
            - `rw`:读写
            - `ro`:只读
