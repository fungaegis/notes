...
## 运行服务
1. 使用命令行： `python3 manage.py runserver`
2. 使用IDE启动
## 创建子应用
1. 使用命令行： `python3 manage.py startapp application_name`
2. 使用IDE，点击tools，选择`Run manage.py Task..`.在该交互环境下，命令行：`startapp application_name`
## 注册子应用
1. 在工程目录下，`setting.py`的`INSTALLED_APPS`中添加application_name
## 路由匹配规则
