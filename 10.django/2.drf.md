...
2. 获取token/更换token
    ...
    2. 序列化器
        ...
        2. `get_token`方法中构造`token`,保存`token`数据
            1. 该方法中调用了`for_user`方法, 实际调用的是`Token`类的方法,创建了`TOKEN`类实例
            2. 实例化`Token`时, 调用`get_token_backend`获取`TokenBackend`类实例
            3. 最终由`TokenBackend`类实例调用`jwt`相关`decode`方法 构建了`Token`令牌
