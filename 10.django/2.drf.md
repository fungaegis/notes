...
1. 如果配置了多个鉴权类,访问该视图时需要满足每一个类的鉴权,否则将失败
...
```
## django认证与drf认证的区别
django自带认证系统,在`AUTHENTICATION_BACKENDS`中可以配置,默认为`ModelBackend`.那drf的认证系统与django自带的有什么区别呢?

`AUTHENTICATION_BACKENDS`: django层认证
1. django自带的认证系统多用来进行登录校验,调用`django.contrib.auth.authenticate`时会遍历`AUTHENTICATION_BACKENDS`中设置的认证系统,直至有一个成功时停止.

`DEFAULT_AUTHENTICATION_CLASSES`: drf层认证
1. 多用来校验后续的认证,如token等.
2. 需要视图的父类为`APIView`才有效
