...
def create(self, validated_data):
    # ...
    user = serializers.CurrentUserDefault()(self)  # TODO 补充doc
    # ....
    return beat
...
3. 如果有设置`list_serializer_class`,则继续执行指定类的`validate`
...
如果使用的视图不是使用`genericAPIView`及其子类,需要手动调用`self.check_object_permissions(request，obj)`
...
class ProductFilter(filters.FilterSet):
    ...
    class Meta:
        model = Product
        fields = ['category', 'in_stock']  # 可以使用的查询字段 fields中的 加上 声明的三个字段

    def range_data(self, queryset, name, value):
        lookup = name + "__range"
...
```py
REST_FRAMEWORK = {
    'EXCEPTION_HANDLER': 'utils.exception_handler.catch_exception',  # 默认使用rest_framework.views.exception_handler
}
```
## 自定义
```py
...
```
## 配置
- CORS_ORIGIN_ALLOW_ALL: 全部允许配置, 与`CORS_ORIGIN_WHITELIST`二选一
- CORS_ORIGIN_WHITELIST: 白名单配置, 与`CORS_ORIGIN_ALLOW_ALL`二选一
- CORS_ORIGIN_REGEX_WHITELIST: 正则表达式的白名单
- CORS_ALLOW_CREDENTIALS: 允许cookie
