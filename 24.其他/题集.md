...
- 视图
    ...
    - ViewSetMixin: 重写 as_view({method: function})
        - Router action
    - ModelViewSet
- 路由rest_framework.routers: SimpleRouter DefaultRouter
    ...
    - action(methods=[], detail, url_parh, url_name)
        - url_path默认方法名 url_name端口名默认`类_方法名`
    - 格式: 路由/路由参数/url_path  路由/url_path
- 权限rest_framework.permissions: IsAuthenticated IsAdminUser IsAuthenticatedOrReadOnly
    - django提供权限 增删改查: add_class change_class delete_class view_class
    - content_type 应用和模型类
    - 用户类权限函数
        - get_user_permissions get_group_permissions get_all_permissions has_perm has_perms has_module_perms
- 限流
- 分页
- 令牌
- 用户系统
