...
_html页面可以通过二次封装bootstrap基模板，一般情况下页面导航来都是公用模块，此时可以封装一个统一的导航栏成为一个新的基类模板_
...
_username & password：数据库的登录凭据；hostname：服务器地址\(远程或本地\)；database：指定需要使用的数据库_
...
_数据库会话能保证数据库的一致性,提交操作使用原子方式把会话中的对象全部写入数据库,避免因部分更新导致数据库不一致_
